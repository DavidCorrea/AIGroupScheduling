...
*Feature index (where to look):* Security roadmap → "Security (roadmap)" below. API consistency & validation → "API consistency, security, and validation". Config & BFF → "Config BFF", "View-scoped config", "Server-side group resolution", "Configuration". Schedule model → "Schedule / recurring events", "Schedule generation algorithm", "Schedule generation & preview". Auth & admin → "User authentication", "Admin panel". Calendar export → "Save in Calendar", "Library skills". UX → "Product and UX clarity", "Dashboard, navigation, public view", "Theme (prefers-color-scheme)". Data → "Member management", "Role management", "Holidays", "Multi-group architecture". Seeds / QA → "Large realistic seed (50 users, 35 groups)". Performance → "Server-rendered cronograma with ISR".

**Server-rendered cronograma with ISR**
- Public cronograma `/:slug/cronograma/:year/:month` is an **async Server Component** that fetches schedule data directly via `buildPublicScheduleResponse` — no client-side fetch waterfall (eliminates JS download → hydrate → useEffect → API call chain). ISR cached with `revalidate = 300` (5-minute fallback TTL). On-demand revalidation: `revalidateCronograma(groupId, month, year)` in `src/lib/public-schedule.ts` calls `revalidatePath` so changes reflect immediately when a coordinator commits, edits, or deletes a schedule. Called from all mutation routes in `src/app/api/schedules/route.ts`, `src/app/api/schedules/[id]/route.ts`, and `src/app/api/schedules/[id]/notes/route.ts`. The API route (`/api/cronograma/[slug]/[year]/[month]`) is kept for other consumers (Google Calendar). `loading.tsx` skeleton shows via Suspense while the server fetches. Error boundary at `[slug]/cronograma/error.tsx` catches server errors.
...
- **Public view**: `/:slug/cronograma` redirects (302) to `/:slug/cronograma/:year/:month` for current month; `/:slug/cronograma/:year/:month` is the canonical schedule view — **async Server Component** that fetches data directly via `buildPublicScheduleResponse` (no client-side fetch waterfall). ISR cached (`revalidate = 300`); on-demand revalidation via `revalidateCronograma()` (`src/lib/public-schedule.ts`) is called in all schedule mutation routes. API `/api/cronograma/:slug`, `/api/cronograma/:slug/:year/:month` (kept for other consumers like Google Calendar). "Guardar en calendario" for **my assignments** lives on **Mis asignaciones** (see below). **SharedScheduleView** is split into `src/components/SharedScheduleView/` (index, types, MonthHeader, DateDetailModal with Radix Dialog, WeekSection, CalendarGrid); same public API.
