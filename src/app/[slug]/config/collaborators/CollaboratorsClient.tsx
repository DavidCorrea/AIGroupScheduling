...
export default function CollaboratorsClient({
  ...
  const [owner] = useState<Owner | null>(initialData.owner);
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      ...
      const filtered = results.filter(
        (u: UserSearchResult) => !existingIds.has(u.id)
      );
  ...
  }, [searchQuery, owner, collaborators, slug]);
  ...
  const performRemove = async () => {
    if (collabIdToRemove == null) return;
    setRemoveInProgress(true);
    try {
      await fetch(`/api/groups/${groupId}/collaborators?collabId=${collabIdToRemove}`, {
        method: "DELETE",
      });
      setCollaborators((prev) => prev.filter((c) => c.id !== collabIdToRemove));
