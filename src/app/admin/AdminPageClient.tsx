...
export default function AdminPageClient({
  ...
  const toggleFlag = async (
    userId: string,
    flag: "isAdmin" | "canCreateGroups" | "canExportCalendars",
    value: boolean,
  ) => {
  ...
  const performDeleteUser = async () => {
    ...
    try {
      await fetch(`/api/admin/users?id=${userIdToDelete}`, {
        method: "DELETE",
      });
  ...
  const performDeleteGroup = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/groups?groupId=${groupToDelete.id}`,
        { method: "DELETE" },
      );
      if (res.ok) {
  ...
  const startImpersonating = async (userId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(
          (data as { error?: string }).error ?? t("errorLoadUsers"),
        );
        return;
      }
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:py-12 sm:px-6 lg:px-8">
        ...
        >
          <h2
            ...
            className="uppercase tracking-widest text-xs font-medium text-muted-foreground mb-4 sm:mb-6"
          >
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <TogglePill
                      ...
                      onChange={(v) =>
                        toggleFlag(user.id, "canCreateGroups", v)
                      }
                    ...
                    <TogglePill
                      ...
                      onChange={(v) =>
                        toggleFlag(user.id, "canExportCalendars", v)
                      }
        ...
        <section
          className="mt-10 sm:mt-12 border-t border-border pt-6 sm:pt-8"
          aria-labelledby="admin-groups-heading"
        >
          <h2
        ...
        <ConfirmDialog
          ...
          message={t("confirmDeleteUser", {
            name: userNameToDelete ?? "este usuario",
          })}
        ...
        <ConfirmDialog
          ...
          message={t("confirmDeleteGroup", {
            name: groupToDelete?.name ?? "este grupo",
          })}
          confirmLabel={t("delete")}
