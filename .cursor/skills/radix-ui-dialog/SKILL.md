...
- **Custom dialogs (EventForm):** Two inline `Dialog.Root` usages for (1) delete event with options (event only vs event and dates) and (2) recalc affected schedules. Both keep Root always mounted and use `open={!!(showDeleteDialog && affectedInfo)}` / `open={!!(showRecalcDialog && affectedInfo)}`. Both use `Dialog.Portal`, `Dialog.Overlay`, `Dialog.Content`, `Dialog.Title`, `Dialog.Description` with ids, and `onEscapeKeyDown` to close.

## How it should be used (Radix best practices)

- **New use cases:** Before adding a custom dialog (multi-option confirm, etc.), check Radix Dialog docs and document the pattern in this skill.
